...
// ALiFe platform treasury — receives the 30% protocol fee
const ALIFE_TREASURY = process.env.NEXT_PUBLIC_ALIFE_TREASURY as `0x${string}`;
const ALIFE_PROTOCOL_FEE_PERCENT = 30; // 30% to ALiFe, 70% to creator

// Stored after first deployment — persist this in your env/db
...
/**
 * Flaunch a token for a new ALiFe agent
 * The token's fee NFT goes into the RevenueManager for automatic 70/30 split
 */
export async function flaunchAgentToken(
  ...
  // Flaunch the token — NFT goes to RevenueManager (not directly to creator)
  // This means fees auto-split: 70% creator / 30% ALiFe
  const hash = await flaunch.flaunchIPFS({
    name: params.name,
    symbol: params.symbol,
    creator: REVENUE_MANAGER_ADDRESS, // NFT → RevenueManager for auto fee split
    creatorFeeAllocationPercent: params.creatorFeeAllocationPercent ?? 80, // 80% to meme stream, 20% to PBW (price support)
    fairLaunchPercent: 0,
    fairLaunchDuration: params.fairLaunchDurationSeconds ?? 30 * 60, // 30 min
    initialMarketCapUSD: params.initialMarketCapUSD ?? 10_000, // $10k
    metadata: {
